...
class SubCommitmentMH {
  ...
  newSubCommitment(SubCommitmentM subCommitmentM) {
    dbHelper.insertData(
        SubCommitConst.subCommitTableName, subCommitmentM.toMap());
  }

  updateSubCommitment(SubCommitmentM subCommitmentM) {
    dbHelper.upDateData(
        SubCommitConst.subCommitTableName,
        subCommitmentM.toMap(),
        SubCommitConst.columReqNo,
        subCommitmentM.toMap()[SubCommitConst.columReqNo]);
  }
  ...
  getSubCommitByReqNo(int reqNo) async {
    List<SubCommitmentM> list = [];
    await dbHelper
        .getDataWhere(
            SubCommitConst.subCommitTableName, SubCommitConst.columReqNo, reqNo)
        .then((value) {
      for (var element in value) {
        list.add(SubCommitmentM.fromMap(element));
      }
  ...
  getSubCommitNotApproved() async {
    List<SubCommitmentM> list = [];
    await dbHelper
        .getDataWhere(SubCommitConst.subCommitTableName,
            SubCommitConst.columIsApproved, 0)
        .then((value) {
      for (var element in value) {
        list.add(SubCommitmentM.fromMap(element));
      }
  ...
  getSubCommitApproved() async {
    ...
    await dbHelper
        .getDataWhere(SubCommitConst.subCommitTableName,
            SubCommitConst.columIsApproved, 1)
        .then((value) {
      for (var element in value) {
        list.add(SubCommitmentM.fromMap(element));
      }
