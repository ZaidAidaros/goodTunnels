...
class OrganizationMH {
  ...
  createOrganization(OrganizationM organizationM) {
    dbHelper.insertData(OrgConst.orgTableName, organizationM.toMap());
  }

  updateOrganization(OrganizationM organizationM) {
    dbHelper.upDateData(
        CodingBlockConst.columOrgCode,
        organizationM.toMap(),
        CodingBlockConst.columCode,
        organizationM.toMap()[CodingBlockConst.columCode]);
  }
  ...
  Future<OrganizationM?> getOrgByPhone(String phone) async {
    return await dbHelper
        .getDataWhere(OrgConst.orgTableName, OrgConst.columOrgPhone, phone)
        .then((value) {
      if (value.isNotEmpty) {
